...
export function processHingeData(data) {
  ...
  return {
    ...
    totalConversations,
    totalUnmatches,
    analytics: generateAnalytics(matches, messages, likes, weMet, blocks),
  };
}

function generateAnalytics(matches, messages, likes, weMet, blocks) {
  return {
    // Existing analytics
    ...
    likesToMatchConversion: getLikesToMatchConversion(likes, matches),
...
function getLikesToMatchConversion(likes, matches) {
  const totalLikes = likes.length;
  const totalMatches = matches.length;

  if (totalLikes === 0) return { rate: 0, matched: 0, unmatched: totalLikes };

  const conversionRate = (totalMatches / totalLikes) * 100;

  return {
    rate: conversionRate,
    matched: totalMatches,
    unmatched: totalLikes - totalMatches,
    totalLikes,
  };
}
